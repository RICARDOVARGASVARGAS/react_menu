...
/**
 * Componente para editar un Asociación
...
const EditGroup = ({ onClose, itemId }) => {
  ...
  useEffect(() => {
    const fetchCarData = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/getGroup/${itemId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/updateGroup/${itemId}`, {
        method: "PUT",
        headers: {
      ...
      console.log(error);
      toast.error("Error al actualizar el Asociación.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/deleteGroup/${itemId}`, {
        method: "DELETE",
      });

      const { data, message, error } = await response.json();
      if (error) {
        toast.error(message || "Error al eliminar el Asociación.");
        return;
      }
      toast.success(message || "Asociación eliminado.");
      onClose();
    } catch (error) {
      console.error("Error al eliminar el Asociación:", error);
      toast.error("No se pudo eliminar el Asociación.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-[90%] md:w-1/3">
            <h3 className="text-lg font-semibold mb-4">
              ¿Estás seguro de que deseas eliminar este Asociación?
            </h3>
