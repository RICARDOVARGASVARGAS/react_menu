 ...
 */
const EditTypeCar = ({ onClose, itemId }) => {
  ...
  useEffect(() => {
    const fetchCarData = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/getTypeCar/${itemId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/updateTypeCar/${itemId}`, {
        method: "PUT",
        headers: {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/deleteTypeCar/${itemId}`, {
        method: "DELETE",
      });

      const { data, message, error } = await response.json();
      if (error) {
        toast.error(message || "Error al eliminar el Tipo de Vehículo.");
        return;
      }
      toast.success(message || "Tipo de Vehículo eliminado.");
      onClose();
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-2 bg-white shadow-lg rounded-lg relative max-h-[80vh] overflow-y-auto">
      ...
      <h2 className="text-2xl font-bold mb-4">Editar Tipo de Vehículo</h2>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
          <div>
            <label className="block text-sm font-semibold">Tipo de Vehículo </label>
            <input
              type="text"
              id="name"
              name="name"
