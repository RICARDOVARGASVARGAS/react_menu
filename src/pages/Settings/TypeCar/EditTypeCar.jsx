...
const EditTypeCar = ({ onClose, itemId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await apiGet(`getTypeCar/${itemId}`);
  ...
  const onSubmit = handleSubmit(async (data) => {
    setIsLoading(true);
    try {
      const response = await apiPut(`updateTypeCar/${itemId}`, data);
      ...
      if (updatedData) {
        ...
        toast.error(message || "No se pudo actualizar el Tipo.");
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await apiDelete(`deleteTypeCar/${itemId}`);
      const { data, message } = response;
      if (data) {
        toast.success(message || "Tipo eliminado.");
        onClose();
      } else {
        toast.error(message || "No se pudo eliminar la Tipo.");
      }
    } catch (error) {
      handleBackendErrors(error, setError);
    } finally {
    ...
    <div className="container mx-auto p-2 bg-white shadow-lg rounded-lg relative max-h-[80vh] overflow-y-auto">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="flex justify-end mt-6 gap-4">
          <ProtectedComponent requiredPermissions={"type.destroy"}>
            <button
              type="button"
      ...
      {showDeleteModal && (
        <DeleteModal
          text="¿Estás seguro de que deseas eliminar esta Tipo?"
